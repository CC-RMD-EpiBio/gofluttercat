...
import (
	...
	npimodel "github.com/CC-RMD-EpiBio/gofluttercat/backend-golang/npi"
...
func LoadItems() []*irtcat.Item {
	cached, err := fs.ReadDir(npimodel.FactorizedDir, "factorized")
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, fn := range cached {
		d, err := fs.ReadFile(npimodel.FactorizedDir, "factorized/"+fn.Name())
		if err != nil {
			log.Fatal(err)
		}
